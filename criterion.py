...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        ...
        # print('target_cls',target_classes)
        # print('src', src_logits)
        ...
        if self.loss_class_type == "ce_loss":
            ...
            # print('hot',target_classes_onehot)
            loss_class = (
                sigmoid_focal_loss(
                    src_logits,
    ...
    def calculate_areas(self, bboxes):
        ...
        :return: A list containing the area of each bounding box.
        """
        # Extract width and height from the bounding boxes
        widths = bboxes[:, 2]
        heights = bboxes[:, 3]

        # Calculate area (width * height) for each bounding box
        areas = widths * heights

        return areas
    
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        """Compute the losses using the size weighting error mechanism for the L1 loss
        """
